...
function calculDiscount(pricePerDay, days) {
    if(days > 10) 
    {
...
{ 
    var rentalDays = numberOfDays(rental.pickupDate, rental.returnDate);
...
function calculDeductibleReduction(rental) 
{
    ...
    {
      return deductibleOptionPriceADay * numberOfDays(rental.pickupDate, rental.returnDate);
    }
    return 0;
}
...
addRowToRentalsTable(rentals[i]);
    ...
    var assistanceAmount = numberOfDays(rentals[i].pickupDate, rentals[i].returnDate);
...
/* UI Section that display result on HTML page */
  ...
  function addRowToVehiculesTable(car) {
      ...
      var row = table.insertRow(1);
      var cell1 = row.insertCell(0);  //Id
      var cell2 = row.insertCell(1);  //Car
